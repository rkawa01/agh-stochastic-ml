...
def new_metaheuristic(
    ...
    # Elite archive
    archive_size = min(10, population_size // 5)  # Increased archive size
    ...
    # Main loop
    reinitialization_interval = budget // 4 # Increased Reinitialization
    ...
    while num_evals < budget:
        ...
        temperature = 1.0 - (num_evals / budget) #Simulated annealing temperature
        ...
        for i in range(population_size):
            if np.random.rand() < exploration_rate:
                ...
                new_x = (inertia_weight * population[i] +
                         personal_learning_coeff * np.random.rand() * (archive_sol - population[i]) +
                         global_learning_coeff * np.random.rand() * (best_x - population[i]))
            ...
            if new_f < fitness[i] or np.random.rand() < np.exp((fitness[i] - new_f) / temperature):
                ...
                new_f = fitness[i]  # Keep the old fitness for updating the archive.
        ...
        if num_evals % (reinitialization_interval // 8) == 0:  # More frequent diversity check
